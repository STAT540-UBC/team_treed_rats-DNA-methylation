...
```{r data_preprocessing, echo=TRUE, eval=FALSE}
...
Within all `r nrow(tabulate)` regions, the methylation of male and female+zeb are the same, and are different compared to female. This is good, because there isn't any filtering - they just happened to end up this way.
...
`r nrow(bsmooth_dt_all_DMR_overlap_final)` of the 34 regions are associated with DE genes
