...
write.table(merged, "RNAseq_all_merged.txt", sep="\t") 
...
###preparing data of p-values
resmat <- data.frame(resmat)
asd <- read.table(file="RNAseq_all_merged.txt", header=T)
resmat$genes <- asd$genes
resmat <- data.frame(resmat$genes, resmat$gene.no, resmat$p.value)
colnames(resmat) <- c("gene.no", "genes", "p.value")
####writing the anova value result
write.table(resmat, "RHgroups_anova.pval.txt", sep="\t")
